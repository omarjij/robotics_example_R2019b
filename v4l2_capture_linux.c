...
#include <sys/ioctl.h>
 ...
 /* for timespec definition in videodev2.h*/
...
static int xioctl(int fd, int request, void *arg)
{
    ...
    } while ((res == -1) && (tries-- > 0) && ((errno == EINTR) || (errno == EAGAIN) || (errno == ETIMEDOUT)));
...
static void initV4L2Device(MW_videoInfo_t *h)
{
    ...
    /* Try selected format */
    if ((fmt.fmt.pix.width != h->imFormat.width) || 
        ...
        case IO_METHOD_MMAP:
            ...
            if (xioctl(h->fd, VIDIOC_DQBUF, &buf) == -1) {
                switch (errno) {
                    ...
                    case EIO:
                    default:
                        printf("Error: Failure reading image buffer. System returned (%d): %s.\n", 
             ...
             if (xioctl(h->fd, VIDIOC_QBUF, &buf) == -1) {
                 printf("Error: Failure returning video buffer to device %s. System returned (%d): %s.\n", 
                         h->devName, errno, strerror(errno));
             }
             break;
    }
...
static void closeV4L2Device(MW_videoInfo_t *h)
{
    ...
    if ( (h != NULL) && (h->fd > 0)) {
        ...
        if (h->v4l2CaptureStarted == 1) {
            /* All buffers are unqueued when capture is stopped */
        ...
        switch (h->ioMethod) {
            ...
            case IO_METHOD_MMAP:
                /* Unmap all buffers */
                for (i = 0; i < h->v4l2BufCount; ++i) {
                    if (munmap(h->frm[i].start, h->frm[i].length) == -1) {
                ...
                req.type   = V4L2_BUF_TYPE_VIDEO_CAPTURE;
                req.memory = V4L2_MEMORY_MMAP;
                if(xioctl(h->fd, VIDIOC_REQBUFS, &req) < 0) {
                    printf("Failed to delete video buffers: System returned (%d): %s.\n",
                            errno, strerror(errno));
                }
                break;
        }
